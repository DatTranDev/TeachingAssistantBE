...
const addSubject = async (req, res) => {
  ...
  const subject = await newSubject.save().catch((err) => {
  ...
  if (sessions)
    await Promise.all(
      sessions.map(async (session) => {
        ...
        await newClassSession.save().catch((err) => {
          return res.status(500).json({
            message: "Internal server error: " + err,
          });
        });
...
const leaveSubject = async (req, res) => {
  ...
  const userIdFromToken = req.user.userId;
  if (
    userIdFromToken != studentId &&
    userIdFromToken != existSubject.hostId.toString()
  ) {
    return res.status(403).json({
      message:
        "Unauthorized action, you are not the host teacher of the subject",
    });
  }
...
const updateSubject = async (req, res) => {
  ...
  if (
    req.body.startDay &&
    req.body.endDay &&
    req.body.startDay.getTime() > req.body.endDay.getTime()
  ) {
    return res.status(400).json({
      message: "Start day must be before end day",
    });
...
const getAvgRating = async (req, res) => {
  ...
  const totalUnderstand = reviews.reduce(
    (acc, review) => acc + review.understandPercent,
    0,
  );
  const totalUseful = reviews.reduce(
    (acc, review) => acc + review.usefulPercent,
    0,
  );
  const totalTeachingMethod = reviews.reduce(
    (acc, review) => acc + parseInt(review.teachingMethodScore),
    0,
  );
  const totalAtmosphere = reviews.reduce(
    (acc, review) => acc + parseInt(review.atmosphereScore),
    0,
  );
  const totalDocument = reviews.reduce(
    (acc, review) => acc + parseInt(review.documentScore),
    0,
  );
  ...
  const avgTeachingMethod =
    totalReviews == 0 ? 0 : totalTeachingMethod / totalReviews;
...
const getStudents = async (req, res) => {
  ...
  const students = userSubjects
    ...
    .map((userSubject) => userSubject.userId);
  return res.status(200).json({
    students: students,
  });
};
...
const getTopParticipants = async (req, res) => {
  ...
  const topParticipants = await DiscussionService.getTopParticipantBySubject(
...
const getTopReactors = async (req, res) => {
  ...
  const top = parseInt(req.query.top) || 5;
  const topReactors = await ReactionService.getTopReactorBySubject(
    req.params.subjectId,
    top,
  );
...
const getTopReviewers = async (req, res) => {
  ...
  const top = parseInt(req.query.top) || 5;
  const topReviewers = await ReviewService.getTopReviewers(
    req.params.subjectId,
    top,
  );
...
const getTopAbsentStudents = async (req, res) => {
  ...
  const top = parseInt(req.query.top) || 5;
  const topAbsentStudents = await AttendRecordService.getTopAbsentStudents(
    req.params.subjectId,
    top,
  );
...
const getSubjectStats = async (req, res) => {
  ...
  const classSessions = await ClassSession.find({ subjectId })
    .select("_id")
    .lean();
  ...
  const cAttends = await CAttend.find({
    ...
    .select("_id")
    .lean();
  const cAttendIds = cAttends.map((c) => c._id);

  const [totalStudents, totalAttendRecords, absentRecords, excusedRecords] =
    await Promise.all([
      UserSubject.countDocuments({ subjectId, role: "student" }),
      AttendRecord.countDocuments({ cAttendId: { $in: cAttendIds } }),
      AttendRecord.countDocuments({
        cAttendId: { $in: cAttendIds },
        status: "KP",
      }),
      AttendRecord.countDocuments({
        cAttendId: { $in: cAttendIds },
  ...
  const presentRecords = totalAttendRecords - absentRecords - excusedRecords;
  const attendanceRate =
    totalAttendRecords > 0
      ? parseFloat(((presentRecords / totalAttendRecords) * 100).toFixed(2))
      : null;

  return res.status(200).json({
    stats: {
      totalStudents,
      totalSessions: cAttendIds.length,
