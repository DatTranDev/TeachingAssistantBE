...
const updateQuestion = async (req, res) => {
  ...
  await Question.findByIdAndUpdate(req.params.id, { isResolved: status })
    .then((question) => {
...
const deleteQuestion = async (req, res) => {
  ...
  if (
    userSubject.role == "student" &&
    userIdFromToken != existQuestion.studentId
  ) {
    await TokenService.deleteTokenByUserID(userIdFromToken);
    return res.status(403).json({
      message: "Unauthorized action",
...
const findBySubjectId = async (req, res) => {
  ...
  if (
    endIndex <
    (await Question.countDocuments({ subjectId: req.params.subjectId }).exec())
  ) {
    results.next = {
      page: page + 1,
      limit: limit,
