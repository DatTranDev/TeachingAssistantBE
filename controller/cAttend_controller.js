...
const findBySubjectId = async (req, res) => {
  ...
  const filter = { classSessionId: { $in: classSessionIds } };
  const [cAttends, total] = await Promise.all([
    CAttend.find(filter).sort({ createdAt: -1 }).skip(skip).limit(limit),
    CAttend.countDocuments(filter),
  ]);

  const checkedExpiredCAttends = await Promise.all(
...
const resetSingleAttendance = async (req, res) => {
  ...
  try {
    const records = await AttendRecord.find({
      cAttendId: req.params.cAttendId,
    });

    await Promise.all(
      records.map(async (record) => {
        const statusEntry = record.listStatus.find(
          (item) => item.index === index,
        );

        const newListStatus = record.listStatus.filter(
          (item) => item.index !== index,
        );
        if (newListStatus.length != record.listStatus.length) {
          record.listStatus = newListStatus;
          record.numberOfAbsence = newListStatus.filter(
            (item) => item.status === "CM",
          ).length;
          record.status =
            record.numberOfAbsence >= existCAttend.acceptedNumber ? "CM" : "KP";
          await record.save();
        }
      }),
    );
...
const updateAcceptedNumber = async (req, res) => {
  ...
  try {
    ...
    await Promise.all(
      attendRecords.map(async (record) => {
        if (record.status != "CP") {
          record.numberOfAbsence = record.listStatus.filter(
            (item) => item.status === "CM",
          ).length;
          record.status =
            record.numberOfAbsence >= req.body.acceptedNumber ? "CM" : "KP";
          await record.save();
        }
      }),
    );
...
const getTopReactors = async (req, res) => {
  ...
  try {
    const topReactors = await ReactionService.getTopReactorByCAttend(
      cAttendId,
      top,
    );
    return res.status(200).json({
...
const getTopParticipants = async (req, res) => {
  ...
  await CAttendService.get(cAttendId);
  try {
    const topParticipants = await DiscussionService.getTopParticipantByCAttend(
      cAttendId,
      top,
    );
    return res.status(200).json({
    ...
    throw new AppError(
      "Error fetching top participants: " + error.message,
      500,
    );
  }
};
